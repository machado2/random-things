import { BodyType, Physics, ShapeType, useRigidBody } from 'use-ammojs';
import { OrbitControls, Cylinder, Sphere, Cone, Plane, Stars } from '@react-three/drei';
...
function Snowflake(props) {
    const radius = 0.1;
    const rbottom = 0.05;
    const vel = new Vector3(rndz(2), rndz(2), rndz(2));
    ...
    return React.createElement(Sphere, { position: props.position, ref: ref, args: [radius] }, material);
}
function Snowflake_(props) {
    const radius = 0.1;
    const rbottom = 0.05;
    const vel = new Vector3(rndz(2), rndz(2), rndz(2));
    const material = React.createElement("meshPhysicalMaterial", { transmission: 0.9, ior: 0.5, color: niceColors[0][Math.floor(Math.random() * 5)] });
...
function Ground() {
    ...
    return React.createElement(Plane, { ref: groundRef, args: [1000, 1000], "position-y": -1, "rotation-x": -halfpi },
